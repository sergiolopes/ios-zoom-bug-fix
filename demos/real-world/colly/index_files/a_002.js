...
Mint.save = function() 
{
	...
	var path	= 'http://colly.com/mint/?record&key=37373335314e346c7047786357306673334453337a31';
...
{
	return /^([^:]+):\/\/([a-z0-9]+[\._-])*(colly\.com)/i.test(url);
};
function Mint_SI_IO_save()
{
	var now		= new Date();
	var then	= now.getTime() + 300;
	var path	= 'http://colly.com/mint/pepper/shauninman/outbound/click.php?'+now.getTime();
	path 		= path.replace(/^https?:/, window.location.protocol);
